...
```{python}
rare_ingredients = ingredients_props[ingredient_props["recipe_count"] < 20, "Ingredient"]
...
#The first bar chart shows that there are a few ingredients that appear in nearly all recipes, while other ingredients are not as common. This shows that all cookie recipes in the database have standard ingredients. The second boxplot compares ratings across recipe sources and whether one source has higher ratings compared to another. Using the boxplot, I can conclude that the recipes found under AllRecipes have a higher overall range of ratings and a higher mean rating compared to the ratings that fall under Epicurious. Also, the miscellaneious recipes are not displayed on the boxplot because no ratings are recorded, therefore they cannot be plotted. Some directions I might want to investigate from here are what are significant factors like cooking time or oven heat that could also affect the cookie ratings?(This is not included in the current data set, just a question in general).
